	...
	$.suggest = function(input, options) {
		...
		function processKey(e) {				
			...
			function displayItems(items) {
				var html = '';
				if (items=='') {
					html='<div class="gray ac_result_tip">请输入关键字查询</div><ul>'+html+'</ul>';
				}
				else {
					var showCount = 10;
					...
					if (html == '') {
						...
						suggest_tip = '<div class="gray ac_result_tip">查找【' + items + '】结果:</div>';
					}
			...
			function selectCurrentResult() {			
				...
				if ($currentResult) {
					...
					if (options.onSelect) {
						options.onSelect.apply($input[0],[$currentResult.attr('rel')]);
					}
				}
